...
class TitleInputComponent extends React.Component {
  ...
  componentDidMount() {
    const rawContent = JSON.parse(
      window.localStorage.getItem('uf-title-input')
    );
    if (rawContent !== null) {
  ...
  saveContent = (content) => {
    ...
    window.localStorage.setItem('uf-title-input', JSON.stringify(rawContent));
  };
  ...
  render() {
    ...
    let className =
      ' text-2xl w-full font-bold outline-none border-b-2 border-[#355A92] text-bold p-1 min-h-[50px]';
